...
class Camera:
    ...
    def __init__(
        self, resolution: tuple[int, int], elevation: int, angle: int, fov: int
    ):
        ...
        if fabs(temp - 1.77) <= 0.01:
            self.ratio = (16, 9)
        elif fabs(temp - 1.6) <= 0.01:
            self.ratio = (16, 10)
        elif fabs(temp - 1.33) <= 0.01:
            self.ratio = (4, 3)
        elif fabs(temp - 1) <= 0.01:
            self.ratio = (1, 1)
        else:
            raise SystemError("Unknown ratio")

        self.vfov = int(
            round(fabs(self.hfov * self.image_height / self.image_width))
        )  # vertical field of view

    # simplified constructor
    @classmethod
    def from_image(cls, resolution: tuple[int, int]) -> "Camera":
        """get a camara without knowing all the attributes"""
        return cls(resolution, 55, 56, 70)

    # default camera settings constructor
    @classmethod
    def from_default(cls) -> "Camera":
    ...
    def get_ratio(self) -> float:
        ...
        return fabs(self.ratio[0] / self.ratio[1])
    ...
    def compute_image_position(self, angle: float, distance: float) -> tuple[int, int]:
        ...
        x = int(
            round((self.image_width / 2) + (angle * (self.image_width / self.hfov)))
        )
        ...
        if distance == 0.0:
            ...
            temp = degrees(atan(self.elevation / distance))

        beta = 90.0 - temp
        beta_diff = beta - (self.angle - (self.vfov / 2))
        y = int(round(self.image_height - (beta_diff / self.vfov * self.image_height)))
    ...
    def _compute_pylon_distance(self, pylon: Pylon) -> float:
        ...
        return float(
            tan(radians(angle)) * fabs(self.elevation - Pylon.get_real_height())
        )
    ...
    def compute_object_image_dimensions(
        ...
        if distance == 0:
            ...
            alpha = degrees(atan(distance / self.elevation))
        alpha_dot = degrees(atan((distance + radius) / self.elevation))
        alpha_diff = fabs(alpha_dot - alpha)
        height_img = int(round(fabs((alpha_diff / self.vfov * self.image_height))))

        gamma = degrees(atan(distance / (self.elevation - radius)))
        beta = 180.0 - (180.0 - gamma) - alpha
        width_img = int(round(fabs(beta / self.vfov * self.image_height)))
    ...
    def _compute_obstacle_distance(self, obstacle: Obstacle) -> float:
        ...
        return float(tan(radians(angle)) * fabs(self.elevation - Obstacle.get_height()))
