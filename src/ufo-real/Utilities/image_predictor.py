...
def analyze_image(
    ...
    matched_nodes, blocked_nodes = render_nodes_match_overlay(
        node_matching, detected_nodes, computed_nodes, graph, img
    )
...
def _compute_image_nodes_from_graph(
    camera: Camera, robot: Robot, graph: Graph
) -> list[VisualNode]:
...
def render_nodes_match_overlay(
    ...
    for n, m in matching:
        if (
            not n == "" and n in computed_nodes and not m == "" and m in measured_nodes
            ...
            if m.__contains__(
                "P"
            ):  # special case for pylon nodes since they are blocked
                ms_colour = RED
                pylons.append(n)
                _find_node_by_str(graph.get_nodes, n).is_blocked()
            else:
                ms_colour = GREEN
                _find_node_by_str(graph.get_nodes, n).is_available()
            ...
            n == "" and not m == "" and m in measured_nodes
        ):  # node found in image that could not get match or is not a node
            ms_x, ms_y = _find_node_by_str(measured_nodes, m).get_coordinates
            ms_colour = ORANGE
        ...
        cv2.ellipse(overlay, (cp_x, cp_y), (cp_w, cp_h), 0, 0, 360, LIGHT_GREY, -1)  # pylint: disable=no-member
        cv2.ellipse(overlay, (cp_x, cp_y), (cp_w, cp_h), 0, 0, 360, BLACK, 1)  # pylint: disable=no-member
        ...
        cv2.putText(  # pylint: disable=no-member
            overlay,
            n,
            (cp_x - SIZE_LARGE, cp_y - cp_h - SIZE_LARGE),
        ...
        # their connection
        cv2.line(overlay, (cp_x, cp_y), (ms_x, ms_y), BLACK, 1)  # pylint: disable=no-member
...
def render_edges(
    ...
    for e in graph.get_edges:
        ...
        if label1 in nodes and label2 in nodes:
            ...
            cv2.line(overlay, (x_1, y_1), (x_2, y_2), colour, 1)  # pylint: disable=no-member

    cv2.addWeighted(overlay, ALPHA, img, 1 - ALPHA, 0, img)  # pylint: disable=no-member


def render_groundplate_obstacle(
    obstacles: list[Obstacle], camera: Camera, img
) -> list[VisualNode]:
...
def _does_line_intersect_ellipse(
